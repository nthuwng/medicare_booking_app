import {
  Modal, Form, Divider, Row, Col, Input, Upload, App, Image
} from "antd";
...
interface IProps {
  ...
  refreshTable: () => void;
  dataSpecialty: ISpecialty | null; // <-- dùng để fill form
}

type FieldType = {
  specialty_name: string;
  description: string;
  // không để icon trong Form values vì mình control bằng state fileListIcon
};
...
const SpecialitesUpdate = (props: IProps) => {
  const { openModalUpdate, setOpenModalUpdate, refreshTable, dataSpecialty } = props;
  ...
  const handleChange = (info: UploadChangeParam) => {
    ...
    if (info.file.status === "done" || info.file.status === "error") setLoadingIcon(false);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <Form form={form} name="form-update-specialty" onFinish={onFinish} autoComplete="off">
          <Row gutter={15}>
            <Col span={24}>
              <Form.Item<FieldType>
                labelCol={{ span: 24 }}
            ...
            <Col span={24}>
              ...
              >
                <Input.TextArea placeholder="Nhập mô tả" rows={4} maxLength={400} />
              </Form.Item>
            </Col>
            ...
            <Col span={12}>
              ...
              {fileListIcon.length === 0 && (
                <div style={{ color: "#ff4d4f" }}>Vui lòng chọn ảnh (hoặc giữ nguyên ảnh cũ).</div>
              )}
            </Col>
          </Row>
        </Form>
