...
const authorizePatient = async (
  ...
  try {
    ...
    const isPatient = await checkPatientByIdViaRabbitMQ(userId);
    if (isPatient.isPatient === false) {
      res.status(403).json({
        success: false,
        message: "Bạn không phải là bệnh nhân, bạn không có quyền truy cập.",
      });
      return;
    }
    next();
